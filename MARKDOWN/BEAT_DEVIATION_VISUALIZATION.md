...
1. **Вычисление координат:**
   - `expectedPosition` — ожидаемая позиция бита; заполняется `BPMAnalyzer::calculateDeviations()` вместе с `deviation`
   - `actualPosition` — фактическая позиция бита
   - Преобразование в экранные координаты (X в пикселях) через `double` — на 192 kHz позиции длинного трека уже не влезают в точные целые `float`
...
- [x] **Статистика отклонений** — `BPMAnalyzer::DeviationStats` (среднее, медианное, максимальное и RMS отклонение, пропуски и дубли долей); возвращается из `calculateDeviations()`
