...
- Исполняемый файл собирает все перечисленные модули; при наличии **thirdparty/qm-dsp** (standalone-копия qm-dsp) подключается библиотека **qm_dsp** и определяется `USE_MIXXX_QM_DSP` для BPM и тональности.
