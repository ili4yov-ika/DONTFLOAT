...
- **macOS (.pkg)** — `tools/macos_build.sh` получил режим `pkg`: после `macdeployqt` собранный `DONTFLOAT.app` со всеми Qt-библиотеками внутри упаковывается `pkgbuild` в установку в `/Applications`. Голый бинарник в `/usr/local` не годится — Qt рядом с ним искать негде.
