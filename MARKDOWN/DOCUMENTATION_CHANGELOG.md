...
## 2026-02-14 (дополнение) - Windows installer, иконка EXE, локализация

### Обновлено

- ✅ **README.md**
  - Добавлен раздел «Сборка Windows installer (NSIS)»: `tools\build_windows_installer.bat`, требования (CMake, Qt, NSIS), генерация иконки из logo.svg
  - В разделе «Интерфейс» добавлено: «Локализация: русский и английский»

- ✅ **DEVELOPMENT_GUIDE.md**
  - Добавлен подраздел «Сборка Windows installer»: вызов скрипта, этапы, результат
  - Добавлен подраздел «Иконка EXE (Windows)»: app.rc, logo.ico, svg_to_ico.py, логика CMake
  - Обновлена структура проекта: tools/, уточнено описание resources/ и docs/

### Синхронизация

- Версия проекта: 0.0.0.1
- Инструменты: tools/build_windows_installer.bat, tools/svg_to_ico.py, nsis_installer.nsi
- Локализация: DONTFLOAT_en_US.ts, DONTFLOAT_ru_RU.ts
...
- ✅ **docs/features.md** — добавлена строка про отмену/повтор (Ctrl+Z/Ctrl+Y) после применения растяжения; дата: февраль 2026
...
- ✅ **README.md** (корневой) — версия 0.0.0.1 и дата (февраль 2026); исправлены метроном (`T`) и метка растяжения (`M`); визуализация отклонений: красный/синий (не зелёный), треугольники, порог 2%; раздел растяжения приведён к системе меток (M, Ctrl+T, ограничения 50 мс); удалён дублирующий блок «Возможности»; добавлен раздел «Консольный режим»; раздел документации переструктурирован (пользовательская / для разработки), убрана битая ссылка на MARKDOWN/FEATURES.md; лицензия и Qt-модули уточнены
...
- ✅ **TIMESTRETCH_FEATURE.md** - документ о функции растяжения времени
...
├── TIMESTRETCH_FEATURE.md       # Функция растяжения времени
