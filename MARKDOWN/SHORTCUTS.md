...
- **Остановка**: без клавиши по умолчанию (кнопка/меню; клавишу можно назначить в диалоге горячих клавиш, id `Stop`)
- **S**: Разрез ноты по каретке воспроизведения (id `SplitNote`)
...
- **Перетаскивание**: горизонтальное панорамирование таймлайна (синхронизация с волной)

### Мышь на PitchGridWidget (ЛКМ)
- **Клик / перетаскивание**: выбор высоты (ноты) и переход к позиции на таймлайне
- **Клик в режиме «Разделить»**: разрез ноты в точке клика (каретка не двигается)

### Панель кнопок под пианороллом
- **«Разделить»**: включает/выключает режим реза (также выключается по `Esc`)
- **«Вдоль сетки» / «Свободный рез»**: рез по ближайшему делению сетки либо точно в позицию каретки/клика
- **«Экспорт MIDI»** (справа на полосе): ноты пианоролла в файл `.mid` (`MidiExporter`); тот же обработчик, что у пункта меню «Файл → Экспорт MIDI…», обе точки входа неактивны без нот
- **«Импорт MIDI»** (рядом с экспортом): референсные ноты из `.mid` серым фоном (`MidiImporter`); после выбора файла — модальный вопрос о тайминге (оставить как есть / подогнать под BPM / выровнять и подогнать). Тональности референса идут потактово отдельной полосой (`KeyModulationStrip` в референсном виде) под полосой тональностей проекта: те же поля по регионам тактов, но серые и без меню выбора
- **S**: разрез ноты под кареткой воспроизведения — глобальный шорткат `MainWindow` (`ApplicationShortcut`), плюс перехват в самом виджете, пока он в фокусе (в плагинах главного окна нет). «Стоп» эту клавишу больше не занимает. Настраивается в диалоге горячих клавиш, id `SplitNote`

### Колёсико на PitchGridWidget
- **Без модификаторов**: вертикальная прокрутка по нотам
- **Ctrl + колёсико**: масштабирование таймлайна (как на волне)
...
|---------|----------|
| Пробел | Воспроизведение/пауза |
| S | Разрез ноты по каретке |
| — | Остановка (клавиша не назначена) |
| T | Метроном |
| ←/→ | Навигация по кадрам |
...
### Интерфейс и сжатие/растяжение
| Клавиша | Действие |
|---------|----------|
| Ctrl+G | Питч-сетка |
| Ctrl+T | Применить сжатие-растяжение по меткам |
| F11 | Полный экран |
| Ctrl+1/2/3 | Панели |
