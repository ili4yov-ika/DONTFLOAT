...
- **Горизонтально:** границы ноты **не** тянутся мышью на roll — длительность меняют **метки stretch** на волне (сегменты между метками).
- **Разрез (✅ 2026-08-16):** нота делится на две части по каретке (клавиша `S`) или кликом в режиме «Разделить» на панели под пианороллом; режим реза — «Вдоль сетки» (snap к делению) или «Свободный рез». Обе части наследуют высоту, разрез в undo/redo (`PitchNoteSplitCommand`). Обратная операция (склейка соседних нот) — в backlog.
