...
Файл (`-f`) декодируется по-настоящему через `QAudioDecoder` в нативном формате и анализируется алгоритмом Mixxx (qm-dsp) — тем же, что и в GUI. На Windows приложение собрано как GUI-процесс, поэтому в консольном режиме оно подключается к консоли родителя (или к перенаправлению `> файл`), чтобы печатать результат.
...
- **--mixxx**: Использовать алгоритм Mixxx (включён по умолчанию)
- **--simple**: Использовать упрощённый алгоритм вместо Mixxx
- **--fast**: Быстрый анализ
...
Файл загружен успешно:
  - Размер: 1723392 сэмплов
  - Частота дискретизации: 44100 Hz
  - Длительность: 39.0792 секунд
...
=== РЕЗУЛЬТАТЫ АНАЛИЗА ===
Определенный BPM: 160.486
Уверенность: 87.1%
Количество обнаруженных битов: 105
Фиксированный темп: Да
Нерегулярные биты: Нет
Среднее отклонение: 1.20%
