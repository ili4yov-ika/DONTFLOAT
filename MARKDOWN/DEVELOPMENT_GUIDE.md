...
Версия — **одна на приложение и все плагины** и живёт в единственном месте:

```cmake
project(DONTFLOAT VERSION 0.1.0.0 LANGUAGES C CXX)
```

Оттуда CMake генерирует `dontfloat_version.h` (шаблон `cmake/dontfloat_version.h.in`);
код берёт `DONTFLOAT_VERSION_STRING`, а установщики читают версию из
`CMakeLists.txt` скриптами сборки. Вписывать номер в исходники руками нельзя —
именно так он однажды и разъехался.
...
- Режим реза нот на пианоролле — `pianoRollCutSnapToGrid` (по умолчанию: «Вдоль сетки»)
- Цветовая схема
...
### Файлы переводов
- **Исходный язык UI**: English (`tr()` / `.ui`)
- **Русский**: ru_RU.ts (EN→RU)
- **Английский**: en_US.ts (EN→EN identity)
- Использование Qt Linguist для редактирования

### Тексты
- Все пользовательские тексты через tr() на английском (whitelist: автоним «Русский»)
