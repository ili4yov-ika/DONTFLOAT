...
Верхняя панель: кнопка открытия файла, список **дорожки** («1/2» — их две, как
на макете), список **формата** (CLAP / VST3 / LV2), список **редакции**
(DONTFLOAT / Scratch / Pitcher), поле **BPM**, **размер такта**, воспроизведение
и стоп. Под ними — **две дорожки** со звуковой волной и тактовой сеткой:
проигранная часть первой залита красным, второй — оранжевым, каретка одна на обе,
под ними время. Ниже — область плагина в красной рамке, куда встраивается
редактор **выбранной** дорожки.
...
### Две дорожки и два экземпляра плагина

У каждой дорожки **свой экземпляр плагина** со своим редактором: видна страница
активной дорожки, соседний экземпляр продолжает жить и считать. Это и есть та
ситуация, ради которой в плагине сделан обмен нотами — ноты соседней дорожки
показываются серым как референс.

- Выбор дорожки: список «1/2», щелчок по самой дорожке или `Ctrl+1` / `Ctrl+2`.
- Открытие файла и правки клипов (`S`, `Ctrl+←/→`, …) идут в активную дорожку.
- Транспорт играет **сумму** дорожек (при двух непустых — с делением на 2, чтобы
  не клиппировало); панель рисует то, что вернул плагин каждой дорожки.
- Списки формата и редакции относятся к активной дорожке: на первой можно
  держать CLAP, на второй — VST3.
- Файл со своей частотой дискретизации подгоняется под частоту сессии (её задаёт
  первая загруженная дорожка) линейной интерполяцией.

Под ARA обе дорожки живут в **одном документе** (по документу на фабрику
плагина): один musical context, у каждой дорожки свои audio source и playback
region. Только так экземпляр второй дорожки видит ноты первой
(`referenceNotesExcluding`) — подробности в `plugins/README.md`. Разные форматы
на дорожках дают разные фабрики и, соответственно, разные документы: ноты друг
друга такие экземпляры не видят, и это правильно.

ARA-путь есть у редакции **Pitcher**: она показывает ноты и референс прямо из
документа, без проигрывания и прогона блоками.

```powershell
# окно с Pitcher на CLAP и загруженным треком
.\dontfloat_mini_daw.exe --format clap --product pitcher --input tests\midi\test_1.wav

# две дорожки сразу: --input в первую, --input2 во вторую (можно тот же файл)
.\dontfloat_mini_daw.exe --format clap --product pitcher --input tests\midi\test_1.wav --input2 tests\midi\test_1.wav
...
Правки идут по клипу под кареткой активной дорожки — щелчок по дорожке
одновременно выбирает и дорожку, и клип. Границы клипов рисуются пунктиром.
