...
- **GiadaPitchGridEngine** (`include/giada_pitchgrid_engine.h`): viewport, пиковая огибающая, линии тактовой сетки, snap к сетке (адаптация Giada `geWaveform`)
- Синхронизация с `WaveformView`: zoom, horizontal offset, каретка, BPM, `gridStartSample`, длина таймлайна (`displaySampleCount()`)
- Таймлайн рисуется на **полную ширину** виджета; подписи нот — overlay справа
- Вертикальный скроллбар (overlay слева) с динамической прозрачностью при приближении каретки
- Поля ввода тональности над панелью
- **По умолчанию скрыта** (`isPitchGridVisible = false`); переключение `Ctrl+G`, состояние в QSettings

### GiadaPitchGridEngine
- Статические утилиты для `PitchGridWidget` (не зависит от сборки `thirdparty/giada`)
- `Viewport::compute`, `buildWaveformPeaks`, `visibleBeatLines`, `snapToBeatGrid`
