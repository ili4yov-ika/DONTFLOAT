...
2. `MainWindow::togglePitchGrid()` переключает `isPitchGridVisible` и вызывает `updatePitchGridLayout()`
...
6. Состояние `pitchGridVisible` сохраняется в QSettings при переключении и при закрытии окна
