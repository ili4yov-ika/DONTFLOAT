...
- **Два режима отображения** (переключаются через `WaveformRenderMode`):
  - **Peaks** (Звуковые пики) — стандартная форма волны по каналам
  - **Spectrogram** (Спектрограмма) — частотная тепловая карта по каждому каналу; строится ленивно (кеш `spectrogramImages`), синхронизирована с зумом/прокруткой
- **`SpectrogramSettings`** — параметры: размер FFT-окна, временное разрешение, частотные полосы, цветовая схема (HeatMap / Grayscale / Cool). Изменяются через `SpectrogramSettingsDialog` в реальном времени.
- Отображение меток цикла A и B
- Отображение меток растяжения (система меток с ромбиками, перетаскивание)
...
### PitchGridWidget
- Отображение piano roll (сетка высот / нот)
- **PianoRollEngine** (`include/pianoroll_engine.h`): viewport, линии тактовой сетки, snap к сетке, подсветка тональности в легенде
- Синхронизация с `WaveformView`: zoom, horizontal offset, каретка, BPM, `gridStartSample`, длина таймлайна (`displaySampleCount()`)
...
### Анализ тональности
1. После загрузки трека `MainWindow` показывает плашку **«Анализировать»** над пианороллом
2. По нажатию кнопки `KeyAnalyzer` получает аудиоданные (фоновый поток через `QtConcurrent`)
3. Применяются алгоритмы анализа хроматического вектора (qm-dsp)
4. Определяется основная и вторичная тональность
5. `MainWindow` обновляет поля тональности и легенду пианоролла; плашка скрывается
6. Пользователь может вручную выбрать тональности через контекстные меню (по умолчанию C Major)
7. Поддержка модуляции: потактовая полоса `KeyModulationStrip` над пианороллом (регионы тактов)
...
4. При перетаскивании метки:
   ...
   - Пересчитываются **коэффициенты растяжения** для соседних участков (только визуальная индикация)
...
- `MainWindow` собирает текущее состояние меток (`MarkerData`/`Marker` из `MarkerEngine`) и аудиоданных и вызывает `TimeStretchProcessor::applyMarkerStretch()` с тонкомпенсацией (Rubber Band R3 через `RubberBandOffline`)
...
1. Пользователь переключает видимость питч-сетки через:
   - Меню **Вид → Показать/Убрать питч-сетку**
   - Горячую клавишу **Ctrl+G**
2. По умолчанию питч-сетка **видна** (`pitchGridVisible` default = true)
3. `MainWindow::togglePitchGrid()` переключает `isPitchGridVisible` и вызывает `updatePitchGridLayout()`
4. Управляется видимость `pitchGridContainer` в `mainSplitter`
5. При скрытии питч-сетки:
   - Скрывается `pitchGridContainer`
   - Волна заполняет всё доступное пространство
6. При показе питч-сетки:
   - Восстанавливается `pitchGridContainer`
   - Вызывается `syncPitchGridFromWaveform()` для актуальных данных
   - Восстанавливаются пропорции сплиттера из QSettings (если сохранены)
7. Горизонтальные отступы волны и питч-сетки одинаковые (`kTimelineHorizontalMarginPx` = 4)
8. Состояние `pitchGridVisible` сохраняется в QSettings при переключении и при закрытии окна
...
- `TimeStretchCommand`: Отмена/повтор применения растяжения по меткам (Ctrl+T)
- `QUndoStack`: Управление историей команд
- Поддержка отмены/повтора (Ctrl+Z / Ctrl+Y)

### Горячие клавиши
- **Основные**: Пробел (воспроизведение), S (разрез ноты по каретке), T (метроном); «Стоп» — без клавиши по умолчанию
...
- **Каретки воспроизведения**: `getCursorXPosition()` и `drawPlaybackCursor()` используют `displaySampleCount()`; питч-сетка масштабирует X через `timelineReferenceWidth`
- **Динамическая прозрачность скролла питч-сетки**: ~90% в покое, ~50% у каретки (зона 10 px)

### Обработка аудио
- **Форматы**: WAV, MP3, FLAC
- **Декодирование**: нативный формат файла → 32-bit float (UInt8/Int16/Int32/Float)
- **Нормализация**: Автоматическая по амплитуде
- **Каналы**: Моно/стерео
- **Частота**: нативная частота файла (без принудительного ресемплинга)
- **Анализ**: BPM, биты, отклонения (expectedPosition, deviation)
- **Сжатие/растяжение по меткам**: TimeStretchProcessor + Rubber Band R3 (`rubberband_offline.cpp`), применение по Ctrl+T
...
- **Метки**: Светло-серые вертикальные линии с ромбиками в центре; для выделенных меток линия и ромб подсвечены белым
- **Визуализация**: Ромбики с цветовыми индикаторами (синие для сжатых, красные для растянутых участков)
- **Управление**:
  - Горячая клавиша `M` для создания метки
  - Перетаскивание ромбика для изменения позиции
  - ЛКМ / ЛКМ+`Shift` для одиночного и группового выделения
  - `Ctrl+A` (при выделенной одной метке) — массовое выделение всех меток
  - Контекстное меню по ПКМ на метке:
    - «Переместить на тактовую сетку» — снэп одной или группы выделенных меток к ближайшим долям (используются `bpm`, `beatsPerBar`, `gridStartSample`)
    - «Удалить метку / выделенные метки»
  - Подстройка тактовой сетки в `WaveformView`: Shift+ЛКМ на волне (`shiftGridBySamples`), кнопки ◀/▶ и Shift+◀/▶ (`gridStartChanged` — метки сдвигаются вместе с сеткой)
- **Автоматическое создание**: Начальная (статичная) и конечная метки создаются при создании первой метки
- **Коэффициенты**: Отображение коэффициентов растяжения между метками
- **Ограничения**: Метки не могут находиться на одной позиции, минимальный сегмент 50 мс, нулевая метка статична; позиция при перетаскивании ограничена соседними метками (реализация в `WaveformView` и структурах `MarkerData`/`Marker` в `MarkerEngine`)

### Оптимизация
- **Отрисовка**: Только видимая область
- **Агрегация**: Сэмплов при масштабировании
- **Кэширование**: Промежуточных вычислений
- **Обновление**: Плавное обновление интерфейса (throttling `scheduleUpdate`, `scheduleRealtimeProcess`)
- **Память**: Эффективное управление аудиоданными
- **Производительность**:
  - Асинхронная обработка BPM
  - Ограничение realtime‑предпросмотра растяжения (для треков длиннее ~5 минут предпросмотр отключается)
  - Отложенное обновление воспроизведения после перетаскивания меток (debounce через таймер в `MainWindow::updatePlaybackAfterMarkerDrag()`)
  - Спектрограмма генерируется один раз ленивым образом и кешируется в `spectrogramImages` (по каналам); при изменении данных или параметров кеш сбрасывается; пиксели заполняются через `scanLine()` для максимальной скорости
- **Новые компоненты**:
  - `SpectrogramSettingsDialog` — немодальный диалог настроек спектрограммы (параметры обновляются немедленно через сигнал `settingsChanged`)
...
- Декодирование через `AudioFileService` (нативный формат, моно для анализа)

Консольный режим загружает и анализирует реальный файл так же, как GUI. Подробнее — [MARKDOWN/CONSOLE_MODE.md](../MARKDOWN/CONSOLE_MODE.md).
...
- `--mixxx`: Алгоритм Mixxx (включён по умолчанию)
- `--simple`: Упрощённый алгоритм вместо Mixxx
- `--fast`: Быстрый анализ
- `--variable-tempo`: Предполагать переменный темп

### GUI: отладочный вывод
По умолчанию диагностика старта (`dontfloat.startup`) отключена. Включить: `--verbose` / `-v` или `QT_LOGGING_RULES=dontfloat.startup.debug=true`.
