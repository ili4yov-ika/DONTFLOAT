...
- `MainWindow` собирает текущее состояние меток и аудиоданных и вызывает движок обработки по меткам
- Движок (`MarkerStretchEngine`) рассчитывает коэффициенты для всех сегментов между метками и обрабатывает их через `TimeStretchProcessor` с тонкомпенсацией
- Результат:
