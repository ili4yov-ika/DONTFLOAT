...
### 🎼 Питч-сетка
По умолчанию **скрыта**; включение: `Ctrl+G` или меню **Вид**.
...
- Поля тональности над пианороллом; по умолчанию **C Major**; второе поле — для модуляции
- После загрузки трека — полупрозрачная плашка **«Анализировать»** для определения тональности (`KeyAnalyzer`)
...
### 🎼 Питч-сетка (скрыта по умолчанию)
...
- **Выбор тональности**: ПКМ по полю тональности; **Анализировать** — автоматический анализ после загрузки
