...
- **Размер окна**: редактор тянется хостом (CLAP — `gui.can_resize` + `get_resize_hints`,
  ...
  Само окно создаётся **безрамочным** (`Qt::FramelessWindowHint`): хост задаёт
  размер внешнего прямоугольника, и с рамкой Qt раскладывал интерфейс по
  клиентской области — она у́же и ниже, отчего справа и снизу оставались полосы.
...
хосту `notifyAudioSourceContentChanged`. Прогресс лежит в модели
(`AraAudioSource::analysisProgress`), и редактор на это время прикрывает
пианоролл плашкой с прогрессбаром — как главное окно.

**Звук без проигрывания**: прочитанная копия дорожки остаётся у источника
(`AraAudioSource::monoSamples`), редактор кладёт её в сессию и дальше рисует
волну, слушает ноты и считает коррекцию, ни разу не получив блок через
`process()`. Мини-DAW под ARA прогон дорожки блоками не делает вовсе.

**Сетка как в DAW**: темп и размер такта читаются из musical context, а начало
такта 1 переводится из времени проекта во время источника по клипу
(`clipsForAudioSource`) — с учётом смещения клипа, взятого куска источника и
растяжения. Поэтому сетка в плагине совпадает с сеткой дорожки.

**Связь «экземпляр → дорожка»** задаётся через playback region: хост назначает
его ролям экземпляра (`ARA::Host::PlaybackRenderer::addPlaybackRegion`,
`EditorView::notifySelection`), и `audioSourceForInstance` находит по нему
источник.

Роли хост раздаёт **позже** привязки, и запасного пути тут нет намеренно.
Раньше редактор в такой момент брал единственный источник документа — а в
начале сеанса он там и правда один, только чужой: соседняя дорожка успела
завестись, своя ещё нет. Ноты соседа становились своими, синими и правимыми,
и дальше уже не менялись. Теперь редактор просто ждёт: опрос повторяется, а
роли приходят. Режим отказа закреплён проверкой
`testEditorWithoutAClipTakesNobodysNotes`.

**Несколько дорожек — один документ.** DAW заводит на плагин один document
controller и сажает на него все экземпляры: у каждого свои region sequence,
audio source, modification и playback region, а musical context общий. Только
поэтому `referenceNotesExcluding(ownSource)` и находит ноты соседней дорожки —
разные документы друг о друге не знают. Мини-DAW делает так же (см. ниже).

Проверки:

- `tests/ara_document_controller_test.cpp` — тест играет роль DAW
  ...
  отдельный случай `testInstancesOnDifferentTracksExchangeReferenceNotes`
  поднимает **два экземпляра плагина** на разных дорожках и проверяет, что
  каждый адресует свой источник и видит ноты соседа как референс.
- `tests/mini_daw_two_tracks_test.cpp` — то же со стороны хоста мини-DAW
  (`AraHostDocument`): две дорожки в одном документе, у каждой свой звук, свои
  ноты и референс с соседней; отдельно проверяется, что удаление дорожки из
  документа не задевает соседнюю.
...
такта — один на документ, а на каждую дорожку (`addTrack`) свои region sequence,
audio source на загруженный WAV, audio modification и playback region на всю
длину. Дорожки с одной фабрикой сидят в одном документе, поэтому их плагины
видят ноты друг друга. `dontfloat_mini_daw --selftest` проходит весь путь:
загрузка модуля → документ → привязка экземпляра → разбор → чтение нот обратно.

Порядок разрушения задан ARA и легко ломается: клип снимается с ролей
(`unbindInstance`), потом уничтожается экземпляр плагина, и только затем
сносятся объекты модели и сам document controller. `AraHostDocument::removeTrack`
и `PluginHost::unload` соблюдают его; фабрика инициализируется один раз на
процесс (счётчик ссылок в `mini_daw_ara_host.cpp` — второй
`initializeARAWithConfiguration` ловится ассертом SDK).
...
**Редактор под ARA** больше ничего не ждёт от захвата блоками: звук, свои ноты,
тактовая сетка и серые ноты соседней дорожки приходят прямо из документа
(`DontfloatPitchEditor::pullFromAraModel`). ARA-путь есть только у редакции
**Pitcher**: у полной редакции и Scratch содержимое редактора другое, и
`setAraBinding` для них — пустая заглушка базового класса.
