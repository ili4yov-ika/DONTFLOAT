...
int main()
{
    ...
    const char* expectedId = Dontfloat::PluginHost::desc().clapId;
    const char* expectedName = Dontfloat::PluginHost::desc().clapName;
    if (!descriptor || std::strcmp(descriptor->id, expectedId) != 0
        || std::strcmp(descriptor->name, expectedName) != 0) {
