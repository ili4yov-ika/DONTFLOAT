...
- **pianoroll_split_test.cpp** - Разрез нот на пианоролле: привязка реза к сетке против свободного, допустимость реза, `PitchNoteSplitCommand` (undo/redo) и реакция `PitchGridWidget` на клик / клавишу `S`; там же референсные ноты из MIDI — рисуются и убираются вместе с `clearReferenceNotes`, но не режутся, и полоса тональностей референса (`KeyModulationStrip` в референсном виде): поля по регионам тактов, клик по ним не открывает меню
