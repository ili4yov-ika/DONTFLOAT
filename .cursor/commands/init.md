...
- **Исходный язык msgid** в коде (`tr()` / `.ui`) и в `.ts` — **английский**; `ru_RU.ts` переводит EN→RU, `en_US.ts` — identity (EN→EN)
- В начале `.ts` и в элементах `<translatorcomment>` у строк выбора языка зафиксирована политика (см. ниже); при `lupdate` проверяйте, что комментарии не потеряны без необходимости

### Выбор языка (инварианты для перевода)
- **Подменю в меню «Настройки»**: строка-источник `Language` во **всех** локалях переводится **той же латиницей** `Language`, не переводить на «Язык» и аналоги
- **Пункты языков в этом подменю**: автонимы **«Русский»** (кириллица) и **«English»** (латиница) во всех поддерживаемых `.ts`; не подменять на «Russian», «Английский», творительный падеж («Русским») и т.п. — это имена опций, а не описательный текст
- **Сообщения в строке состояния** после смены языка: source `Language: English` / `Language: Russian`; в `ru_RU.ts` для `Language: English` перевод = source, для `Language: Russian` — `Язык: Русский`
- Подробные пояснения к конкретным `message` дублируются в `translations/*.ts` в `<translatorcomment>`

### Тексты
- Все пользовательские тексты через tr() **на английском** (кроме автонима «Русский»)
